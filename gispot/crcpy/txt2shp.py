...
def points_genarator(txt_file):
	...
	while input_data:
		...
		if not line_closed: # 为空时
			...
			flag_num = int(row_num[0])  # 1
			# 第二列数字
			flag_row_num2 = row1[1]  # '1'
		elif int(row_num[0]) > flag_num:
			if row_num2 != flag_row_num2: # 第二列出现不一样的，新一轮开始
				flag_row_num2 = row_num2
				polygon_list.append(line_closed)
				line_closed = []
			# 未开始新一轮就接着上一个，如果开始新一轮那么这里就是第一个起始点
			line_closed.append(row1)
		elif int(row_num[0]) == flag_num: # 新一轮开始
			polygon_list.append(line_closed)
			line_closed = []
			line_closed.append(row1)
	if line_closed:
