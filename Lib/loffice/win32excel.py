...
xlsx_list = recur_search(ur"I:\高标准农田上图入库\63嘉陵区高标准农田上图入库项目",[u"xlsx",u"xls",u"XLSX",u"XLS"],recur=True)
# xlsx_list = recur_search(ur"G:\正安县\正安县分布图\成果\标志牌图\3.自然资源部门",[u"xlsx",u"xls",u"XLSX",u"XLS"],recur=True)

# 筛选出隐藏备份文件
xlsx_list2 = [i for i in xlsx_list if "~$" not in i]
print "xlsx_list2:", xlsx_list2

GZDG_value = []
# dir_p = ur"G:\夹江\新建文件夹"
# all_GZDG = os.listdir(dir_p)
for ii in xlsx_list2:
	try:
...
# 复核表
path_2 = ur"G:\正安县\嘉陵“十二五”以来高标准农田建设评估复核统计表.xlsx"
# path_2 = ur"J:\高标准农田\夹江\新建文件夹\附件：夹江“十二五”以来高标准农田建设评估复核统计表（夹江）(2).xlsx"
main_2(path_2,10,GZDG_value)