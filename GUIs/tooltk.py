...
class Tooltk(object):
	...
	def __init__(self):
		...
		self.window.iconbitmap(default=os.path.dirname(__file__) +"/Icons/toolbox.ico")
	...
	def create_frames(self):
			...
			self.frame_side_bar.pack(side="right", anchor="e", expand = False,
									 fill="y")
			self.frame_major.pack(side="top", anchor="center",expand=True, fill=
								  "both")
			# self.frame_middle_bar.pack(side="top", anchor="center",expand=True, fill ="both")
			self.frame_bottom_bar.pack(side="top", anchor="center",expand=False,
									   fill ="x")
				# expand=False, fill ="x" 表示不会随着界面变大而变大，但是在
					# x轴（左右）方向上会拉伸
			self.text = tk.Text(self.frame_side_bar,width = "70",bg = "Tan",
								)
			...
			# 		 ).grid(row = 1)
			# tk.Label(self.frame_side_bar,
			# 		 text=u"一大推处理结果方法辅导费多付多付多付多付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付付",
			# 		 wraplength = "100",
			# 		 height="0",
	...
	def single_file_block(self):
		...
		self.label_1 = tk.Label(self.frame_major, text="untitled 1",
								bg=self.color3)
		...
		self.input_box1 = tk.Entry(frame_one, textvariable=input_msg1,border = 2
								   ,relief = tk.GROOVE)
		self.input_box1.pack(side=tk.LEFT,anchor=tk.W, expand= True, fill = tk.X,
							padx=15)
		# input_msg.set(one_file_path)
		return 1
	
	def single_dir_block(self):
		...
		self.label_2 = tk.Label(self.frame_major, text="untitled 2",
								bg=self.color3)
		...
		# Entry
		input_msg1 = tk.StringVar()
		self.input_box2 = tk.Entry(frame_one, textvariable=input_msg1, border=2,
								   relief=tk.GROOVE)
		self.input_box2.pack(side=tk.LEFT, anchor=tk.W, expand=True, fill=tk.X,
							 padx=15)
		# input_msg.set(one_file_path)
		return 1
	
	def get_int_block(self):
		"""主Frame中的功能块之一，直接通过Entry获取Int"""
		self.label_3 = tk.Label(self.frame_major, text="untitled 2",
								bg=self.color3)
		...
		# Entry
		input_msg1 = tk.StringVar()
		self.input_box3 = tk.Entry(frame_one, textvariable=input_msg1, border=2,
								   relief=tk.GROOVE)
		# , state = "readonly"
		self.input_box3.pack(side=tk.LEFT, anchor=tk.W, expand=True, fill=tk.X,
							 padx=15)
		# input_msg.set(one_file_path)
		return 1

	def get_Entry_fromblock(self):
		"""获取两个Entry值"""
		got_msg1 = self.input_box1.get().decode("cp936")
		got_msg2 = self.input_box2.get().decode("cp936")
